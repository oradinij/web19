...
/**
 * aggiunge un medico al database GET /api/medico?email=?pwd=?spec=?
 */
@WebServlet("/api/medico")
public class ApiMedico extends HttpServlet {
	...
	public void inserisciMedico(String email, String password, int idSpecializzazione ){
		...
		try {
			...
			stmt.executeUpdate();
			Logger.log("Aggiunto medico :\n\temail %s\n\tpassword hash: %s\n\tid specializzazione: %i", email, hashedPassword, idSpecializzazione);
		}
		catch (Exception e) {
			e.printStackTrace();
		}
